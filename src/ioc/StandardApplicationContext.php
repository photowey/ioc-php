...
/**
 * {@code StandardApplicationContext} 实现 {@link ApplicationContext}
 * 是 {@code IOC} 容器的具体实现
 *
 * @author photowey
 * @date 2022/04/02
...
class StandardApplicationContext implements ApplicationContext
{
    ...
    /**
     * @var array {@code bean} name 列表
     */
    protected $handle_register_bean_name = array();
    ...
    public function register(string $bean, $instance, bool $allowDuplicate = false)
    {
        ...
        // 销毁的时候 - 可能会使用
        $handle_register_bean_name[$bean] = $bean;
     ...
     * @throws Exception
     */
    protected function parseDependencies(array $parameters): array
    {
        $dependencies = [];
        $beanNameGenerator = $this->beanNameGenerator();
     ...
     * @throws Exception
     */
    protected function resolveDefaultValue($parameter)
    {
     ...
     * @throws Exception
     */
    protected function beanNameGenerator(): DefaultBeanNameGenerator
    {
    ...
    public function __destruct()
    {
        unset($this->ioc);
        unset($this->mappings);
        unset($this->handle_register_bean_name);
    }
